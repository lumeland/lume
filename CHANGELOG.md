...
- New task `lume`, to run `deno task lume`. This deprecates the `build` and `serve` tasks (they will be removed probably in `1.10.0`). The idea is all Lume commands to be executed by a single point (like `deno task lume --serve`) which makes it more portable without install anything.

### Changed
- The `build` task generated by Lume in `deno.json` no longer include the Lume url 
